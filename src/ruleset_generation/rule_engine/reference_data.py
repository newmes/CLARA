...
# ---------------------------------------------------------------------------
def _mortality_channels(disease_prop: float, toxicity_prop: float, unrelated_prop: float) -> dict:
    """Build mortality model channels in GT proportion_of_deaths format."""
    return {
        "malignant_disease": {"proportion_of_deaths": disease_prop, "description": "deaths from malignant disease"},
        "drug_toxicity": {"proportion_of_deaths": toxicity_prop, "description": "deaths from drug toxicity"},
        "unrelated_ae": {"proportion_of_deaths": unrelated_prop, "description": "deaths from AE unrelated to study drug"},
    }


_DEFAULT_MORTALITY = {
    "baseline_annual_mortality": 0.30,
    "channels": _mortality_channels(0.90, 0.03, 0.04),
}

MORTALITY_MODELS = {
    "lung": {"baseline_annual_mortality": 0.45, "channels": _mortality_channels(0.93, 0.02, 0.03)},
    "melanoma": {"baseline_annual_mortality": 0.25, "channels": _mortality_channels(0.88, 0.04, 0.05)},
    "breast": {"baseline_annual_mortality": 0.20, "channels": _mortality_channels(0.87, 0.03, 0.06)},
    "myeloma": {"baseline_annual_mortality": 0.25, "channels": _mortality_channels(0.85, 0.05, 0.06)},
    "leukemia": {"baseline_annual_mortality": 0.35, "channels": _mortality_channels(0.82, 0.07, 0.06)},
    "lymphoma": {"baseline_annual_mortality": 0.20, "channels": _mortality_channels(0.84, 0.05, 0.06)},
    "prostate": {"baseline_annual_mortality": 0.20, "channels": _mortality_channels(0.88, 0.02, 0.06)},
    "hepatocellular": {"baseline_annual_mortality": 0.55, "channels": _mortality_channels(0.92, 0.03, 0.03)},
    "renal": {"baseline_annual_mortality": 0.30, "channels": _mortality_channels(0.89, 0.03, 0.05)},
    "urothelial": {"baseline_annual_mortality": 0.40, "channels": _mortality_channels(0.90, 0.03, 0.04)},
    "ovarian": {"baseline_annual_mortality": 0.30, "channels": _mortality_channels(0.89, 0.03, 0.05)},
    "pancreatic": {"baseline_annual_mortality": 0.70, "channels": _mortality_channels(0.94, 0.02, 0.02)},
    "glioblastoma": {"baseline_annual_mortality": 0.60, "channels": _mortality_channels(0.93, 0.02, 0.03)},
    "cholangiocarcinoma": {"baseline_annual_mortality": 0.55, "channels": _mortality_channels(0.92, 0.03, 0.03)},
}
...
# ---------------------------------------------------------------------------
# Disposition model defaults — independent hazard rates
# ---------------------------------------------------------------------------
DISPOSITION_DEFAULTS = {
    "independent_hazards": {
        "consent_withdrawal": {
            "daily_probability": 0.000142,
            "description": "estimated from typical trial dropout",
        },
        "physician_decision": {
            "daily_probability": 0.000142,
            "description": "estimated from typical trial dropout",
        ...
        "ae_discontinuation": {
            "daily_probability": 0.000426,
            "description": "estimated from typical AE-driven withdrawal",
        },
    }
}
