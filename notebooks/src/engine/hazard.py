...
def compute_daily_mortality(day, active_aes, tumor_status, ecog, treatment_discontinued=False, response_onset_day=None, risk_config=None, cumulative_doses=None, discontinuation_day=None, is_drug_held=False, has_active_conmed=False):
    ...
    # Active intervention reduces treatment toxicity mortality risk
    if is_drug_held and tt_mult > 1.0:
        tt_mult = 1.0 + (tt_mult - 1.0) * 0.5   # 50% reduction
    if has_active_conmed and tt_mult > 1.0:
        tt_mult = 1.0 + (tt_mult - 1.0) * 0.75   # 25% reduction
