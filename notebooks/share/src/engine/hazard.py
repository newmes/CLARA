...
def compute_daily_mortality(day, active_aes, tumor_status, ecog, treatment_discontinued=False, response_onset_day=None, risk_config=None, cumulative_doses=None, discontinuation_day=None):
    ...
    # When severity_weight=1: full HR applies (critically ill patient)
    # When severity_weight=0.15: HR^0.15 applies (stable responding patient)
    # This avoids double-counting: ECOG is in HR AND sw_ecog, but sw only
    # scales how much the HR matters, preventing multiplicative explosion.
    log_base = math.log(max(daily_base, 1e-12))
    log_hr = math.log(max(hr, 1e-12))
    daily_risk = math.exp(log_base + severity_weight * log_hr)

    # Guaranteed minimum: underlying disease always carries some risk
